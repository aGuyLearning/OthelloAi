...
public class AdversaryLearning {
  ...
  public List<AdversaryTrainingExample> executeEpisode(int iteration) {
    ...
    int currentPlayer = Game.PLAYER_BLACK;
    ...
    while (currentGame.isRunning()) {
      ...
      currentPlayer = currentPlayer == OthelloModel.PLAYER_BLACK ? OthelloModel.PLAYER_WHITE : OthelloModel.PLAYER_BLACK;
    }
  ...
  int chooseNewMoveAction(Set<Integer> validMoveIndices, INDArray normalizedActionProbabilities, OthelloModel currentGame) {
  ...
  void updateMonteCarloSearchRoot(OthelloModel game, int moveAction) {
  ...
  List<MultiDataSet> createMiniBatchList(List<AdversaryTrainingExample> trainingExamples) {
    ...
    for (int currentBatch = 0; currentBatch < batchNumber; currentBatch++) {
      ...
      INDArray probabilitiesLabels = Nd4j.zeros(batchSize, initialGame.getNumMoves());
      ...
      if (currentBatch >= batchNumber - 1) {
        ...
        gameInputBoardStackShape[2]);
        probabilitiesLabels = Nd4j.zeros(lastBatchSize, initialGame.getNumMoves());
      ...
      for (int batchExample = 0, exampleNumber = currentBatch * batchSize;
        ...
        INDArray actionIndexProbabilities = Nd4j.zeros(initialGame.getNumMoves());
